console.log('[Ruslan Bessmertniy] >> Loading bot. Please, wait!');
...
const donate_key = '          '; //сюда вставляешь токен //1qwerty1
const bot_owner = 522736453;
let giving = false;
var wall_to_send = '';
...
vk.setOptions({ token:
'39c171842326e55ecd0d6012e1a7c3743a06276fa1667f1ef5d9579d31c4ebb3b8e23cb98ce51e42d1c7e', pollingGroupId: 176315398 }); //в token вставляешь токен, а в polingGroupId ид своей группы цифровой    //1qwerty1
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[public174970996\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[public174970996\|(.*)\]/ig, '').trim(); //меняешь паблик на свой и ид на свой тоже //1qwerty1
...
cmd.hear(/^(?:eval)\s([^]+)$/i, async (message, bot) => {
	if(message.senderId !== 522736453| message.senderId !== 522736453) return;
...
cmd.hear(/^(?:dev)\s([^]+)$/i, async (message, bot) => {
	if(message.senderId !== 483398970| message.senderId !== 483398970) return;
...
🎭Викторина/Викторина [ответ/подсказка]
💣 Поле [сумма/1-3]
🥛 Стаканчик [1-3] [сумма]
👔 Работа - список работ
 🔨 Работать
...
/*cmd.hear(/^(?:донат)$/i, async (message, bot) => {
	...
	За донатом писать @deniz_mm(Мне)`);
});*/
...
if(message.senderId !== 205439053| message.senderId !== 205439053) return;
...
▄ Админ ${message.user.tag} выдал вам ${utils.sp(message.args[2])}$!
...
vk.updates.hear(/^(?:setmoney)\s?([0-9]+)?/i, message => {
	...
	if(level < 1) return message.send(`🔸 ➄ Вы не администратор`);
	if(level == 4){
		if(!message.args[1] || message.args[1] < 0 || message.args[1] > 1000000000000000000000000000000000000) return message.send(`💰 ➄ Пример: 'setmoney [1-100000000000000000000000000000]'`);
		message.user.balance += Number(message.args[1]);
	}
	return message.send(`💰 ➄ Вы выдали себе ${spaces(message.args[1])}$`);
});
...
💰 Баланс: ${utils.sp(message.user.balance)}$`);
	else if(message.args[2] <= message.user.balance)
	{
		...
		if(message.args[2] + message.user.limit.sent > 50000000000) return bot(`доступно для отправки: ${utils.sp(50000000000 - message.user.limit.sent)}$`)
		let user = users.find(x=> x.uid === Number(message.args[1]));
		if(!user) return bot(`неверный ID игрока`);
...
${message.user.transport.car === 26 ? '🔹' : '🔸'} 27. Вишневая семерка (50.000.000.000.000.000$)
...
cmd.hear(/^(?:репорт|реп|rep|жалоба)\s([^]+)$/i, async (message, bot) => {
	...
	vk.api.messages.send({ user_id: 205439053, forward_messages: message.id, message: `Player id: ${message.user.uid}` }).then(() => {
...
cmd.hear(/^(?:ответ)\s([0-9]+)\s([^]+)$/i, async (message, bot) => {
	if(message.senderId !== 205439053) return;
...
cmd.hear(/^(?:restart)$/i, async (message, bot) => {
	if(message.senderId !== 522736453) return;
...
cmd.hear(/^(?:рестарт)$/i, async (message, bot) => {
	if(message.senderId !== 522736453) return;
	await bot(`бот уходит в перезагрузку, Иван.`);
